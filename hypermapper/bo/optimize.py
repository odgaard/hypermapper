...
def optimize_scipy(
    ...
    non_real_parameter_indices = [
        i for i in range(param_space.dimension) if i not in real_parameter_indices
    ]
    ...
    def f_wrapper(x: np.ndarray):
        ...
        grad = (
            torch.autograd.grad(aq_val, x)[0]
            .contiguous()
            .view(-1)
            .cpu()
            .detach()
    ...
    res = minimize(
        f_wrapper, args=(), x0=x0, bounds=bounds, jac=True, options={"gtol": 1e-8}
    )
...
def optimize_acq(
    ...
    acquisition_function_parameters = {
        ...
        "feasibility_threshold": (
            np.random.choice(
                settings["feasible_output"]["feasibility_threshold"]
                * 0.1
                * np.arange(11)
            )
    ...
    ### LOCAL SEARCH
    n_start_points = min(
        settings["local_search_starting_points"], rs_acquisition_values.shape[0]
    )
    ...
    if settings["local_search_from_best"]:
        ...
        start_configurations = torch.cat(
            (start_configurations, previous_configurations), 0
        )
    ...
    for start_configuration, start_value in zip(start_configurations, start_values):
        ...
        while value > previous_value + 1e-12:
            ...
            if (not param_space.only_real_parameters) or (
                not param_space.use_gradient_descent
            ):
        ...
        best_configurations = torch.cat(
            (best_configurations, configuration.unsqueeze(0))
        )
    ...
    try:
        ...
        for i, p in enumerate(param_space.parameters):
            normalized_start_configurations[:, i] = (
                normalized_start_configurations[:, i] - p.min_value
        ...
        start_conf_dists = torch.tensor(
            [
                [
                    torch.norm(
                        normalized_start_configurations[i]
                        - normalized_start_configurations[j]
                    )
                    for i in range(start_configurations.shape[0])
                ]
        ...
        start_val_dists = torch.tensor(
            [
                [
                    torch.abs(start_values[i] - start_values[j])
        ...
        sys.stdout.write_to_logfile(
            f"x diffs: {start_conf_dists[np.nonzero(start_conf_dists)].min()}-{start_conf_dists.max()}\n"
        )
        sys.stdout.write_to_logfile(
            f"y diffs: {start_val_dists[np.nonzero(start_val_dists)].min()}-{start_val_dists.max()}\n\n"
        )
    except:
        pass
    ########################
